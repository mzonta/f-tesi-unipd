...
visualizza_tempo=0; % Tempi parziali
...
Ni=2*ones(1,Ns); %hp di Ni costante per ogni span
...
while (stop>0)
   ...
   if (visualizza_tempo~=0)
      ...
      disp('Tempo per il calcolo di Rs e Rss');
      disp(tempo);
   ...
   if (visualizza_tempo~=0)
      ...
      disp('Tempo per il calcolo delle normali esterne')
      disp(tempo);
   ...
   Mask_G=zeros(R,C);
   Mask_G=sparse(double(roipoly(Mask_G,G(:,1),G(:,2))));
   
   if (Li>0)
      
      % Solo una parte della regione interna delimitata dalla banda Li
      Mask_Gi=zeros(R,C);
      Mask_Gi=sparse(double(roipoly(Mask_Gi,Gi(:,1),Gi(:,2))));
      
      % Maschera per area interna
      Mask_int=sparse(Mask_G.*not(Mask_Gi));
   ...
   if (Le>0)
      ...
      Mask_Ge=zeros(R,C);
      Mask_Ge=sparse(double(roipoly(Mask_Ge,Ge(:,1),Ge(:,2))));
   
      % Maschera per area esterna
      Mask_est=sparse(Mask_Ge.*not(Mask_G));
   ...
   for nv=1:nvalori
      ...
      Im_int=sparse(Mask_int.*ImIn(:,:,nv)); % (*) O4
      % Matrice di pixel della parte di immagine esterna allo snake
      Im_est=sparse(Mask_est.*ImIn(:,:,nv)); % (*) O4
      ...
      if (media_varianza==1)
         ...
         Media_int(nv)=mean2(Im_int(ind_int));
         % Media dei valori esterni allo snake
         Media_est(nv)=mean2(Im_est(ind_est));
   ...
   if (visualizza_tempo~=0)
      ...
      disp('Tempo per indici,aree,medie-varianze');
      disp(tempo);
   ...
   for i=1:Npunti
      ...
      g_dt_i=g_dt*(1-g_curv*abs(Vks(i)));
      
      %Gnc(i,:)=G(i,:)+g_dt*(dG-g_curv*Vks(i))*Mne(i,:); %(Npunti x 2)
      Gnc(i,:)=G(i,:)+g_dt_i*dG*Mne(i,:); %(Npunti x 2)
   ...
   if (visualizza_tempo~=0)
      ...
      disp('Tempo per le nuove coordinate');
      disp(tempo);
...
disp('Tempo totale di esecuzione = ');
disp(tempo_totale);
