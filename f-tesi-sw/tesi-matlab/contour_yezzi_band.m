...
visualizza=3; % Stato di visualizzazione risultati parziali (per DEBUG)
...
g_dt=10e5; %.05e5; %.4e5; % media con area
...
while (stop>0)
   ...
   if (visualizza==3)
      ...
      Gng=rot_tras(G,Npunti,rot,tras);
      plot(Gnc_cg(:,1),Gnc_cg(:,2),'k+');
      plot(Gng(:,1),Gng(:,2),'r');
      title('Confronto tra Gnc e G');
