...
% *) ImIn: matrice intensit immagine
...
% *) g_dt: fattore di scala fra l'intensit della forza e lo spostamento effettivo.
%          [dell'ordine 1e5]
% *) g_curv: fattore di scala per peso curvatura.
%          [dell'ordine 100]
% *) n_W: dimensione finestra per la misura del valore d'intensit del
...
%                   Ogni span  caratterizzato dalla shape matrix M 
%
%
...
% -3) Se il numero di p.ti non  ritenuto sufficiente se ne inseriscono 
%     automaticamente, secondo qualche criterio, altri in posizioni intermedie.
%Oss.: verificare se  possibile qualche metodo per inizializzazione automatica
%      o semi-automatica.
...
%     di regolarit (smooth) sulla curva .
...
% -9) Si ripete dal passo 6) finch non si ritiene snake in equilibrio.
...
%         condiziona la convergenza in quanto pu portare lo snake a "saltare"
...
%         perch il calcolo successivo delle normali esterne  basato sull'hp
...
imshow(ImIn);
hold on;
% - Roto-traslazione inversa: per le propriet della trasformazione considerata
%   l'inversa  uguale alla traslazione diretta.
...
while (stop==0)
   ...
   for i=1:Npunti
      ...
      %      per la curva, ovvero l'orientamento della superficie  positivo se uscente
      %      dallo schermo.
      ...
      %                     
      %     quindi si pu considerare per il solo modulo e segno quelli dati da ks   
   ...
   %   Le()*Mne() nei tratti ad alta curvatura, cio si mantenga sempre 
   ...
   Mask_G=double(poly2mask(G_I(:,1),G_I(:,2),R,C));
   ...
   if (dLi>0)
      ...
      Mask_Gi=double(poly2mask(Gi(:,1),Gi(:,2),R,C));
   ...
   if (dLe>0)
      ...
      Mask_Ge=double(poly2mask(Ge(:,1),Ge(:,2),R,C));
   ...
   for i=1:Npunti
      ...
      for nv=1:ncomp
         ...
         % G_I  definita nel sistema di riferimento "Immagine".
         rpx=ceil(G_I(i,2));
         cpx=ceil(G_I(i,1));
      ...
      % Da indicazione della "forza" a cui  soggetto il campione i-esimo dello
      % snake; dalla loro entit posso ricavare informazioni sulla complessit
   ...
   %        In base alle propriet geometriche dello snake-spline (smoothness - loop)         
   % ===================================================================================
   
   % Ogni <npassinuoviCP> si aggiorna la complessit della curva a B-spline.
   npassinuoviCP=5;
   ...
   if (rem(contatore,npassinuoviCP)==0)
      ...
      fprintf(1,'Fase di aggiornamento complessit \n');
   ...
   if ((rem(contatore,npassi_stop)==0) & visualizza>=0)
...
% O2) La matrice A (e quindi B)  costante se non varia il numero di campioni considerato; 
...
%     quando si verifica che si  prossimi alla convergenza.

% O4) Considerare MATRICI SPARSE dove conveniente
%     Vedere se  conveniente introdurre test su sparseness dopo operazioni con full-matrix
