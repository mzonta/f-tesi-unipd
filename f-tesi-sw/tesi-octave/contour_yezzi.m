...
visualizza=0; % Stato di visualizzazione risultati parziali
...
set(gcf,'Name','SNAKE');
title('Individuare sull''immagine i punti di riferimento del contorno. Per terminare premere >Esc<');
...
while (stop>0)
   ...
   % -6a) Calcolo dei parametri statistici interni ed esterni allo snake.
   
   % Considerando il poligono che approssima lo snake, i cui vertici sono costituiti
   % dai punti-campioni dello snake, si ottiene la maschera che individua i pixel
   % ed esterni allo snake.
   
   Mask_int=zeros(R,C);
   % Maschera per area interna
   Mask_int=double(roipoly(Mask_int,G(:,1),G(:,2)));
   % Maschera per area esterna = maschera complementare
   Mask_est=double(not(Mask_int));
   ...
   for i=1:Npunti
      ...
      % -6b) Calcolo della normale esterna (vne) allo snake-spline nell'i-esimo punto. 
      ...
      vne=-[rs_n(2), -rs_n(1)]; %(1 x 2) vettore riga
            
      % -6c) Calcolo della curvatura con segno nel punto i-esimo dello snake.
      ...
      ks=(rs(1)*rss(2)-rs(2)-rss(1))/(norm(rs)^3);
         
      % -6d) Le coordinate dei nuovi punti di contatto per lo snake al passo k+1-esimo 
      ...
      for nv=1:nvalori
         ...
         dG=dG_M_A; % SCELTA CRITERIO-PARAMETRO
         %dG=dG_V_A;
      ...
      %if (abs(dG)>.4e-5)
         Gnc(i,:)=G(i,:)+g_dt*(dG-g_curv*ks)*vne; %(Npunti x 2)
         %Gnc(i,:)=G(i,:)+g_dt*dG*vne; %(Npunti x 2)
      %else
         %Gnc(i,:)=G(i,:);
      %end
...
% Chiude tutte le finestre precedentemente attivate tranne la prima
close(2:n_figure-1);
